...
class Solution{
public:
    string Ino(Node* root,unordered_map<string,int>&mp)
    {
        if(root == NULL)
        {
            return "";
        ...
        {
            left = '(' + Ino(root->children[0],mp) + ')';
        }
        string st = to_string(root->data);
        int cnt = 0;
